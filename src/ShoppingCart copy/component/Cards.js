...
import { Add, remove } from '../slice/Reducer';
            ...
            <h1>Dairy, Bread & Eggs</h1>
                ...
                <div className=' row col-12 justify-content-around'>
                    {userDetils.map((product) => (
                        <Card border="success" style={{ width: '15rem' }} className="carthover m-2 " key={product.id}>
                            <Card.Img variant="top" src={product.image} onClick={()=>navigate('/cartshowdata')}/>
                            <Card.Body >
                                <Card.Text style={{ textDecoration: 'none' }} onClick={()=>navigate('/cartshowdata')} >{product.title}</Card.Text>
                                <div className='d-flex justify-content-between'>
                                    <Card.Text onClick={()=>navigate('/cartshowdata')}>₹{product.price}</Card.Text>
                                    <Card.Text onClick={()=>navigate('/cartshowdata')}>{product.weight}</Card.Text>
                                </div>
                                <div className='d-flex justify-content-between'>
